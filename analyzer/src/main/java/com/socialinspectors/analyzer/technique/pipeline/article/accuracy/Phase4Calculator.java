...
public class Phase4Calculator extends PhaseCalculator {

	private static final String PHASE = "phase4";

	public Phase4Calculator() throws IOException, InterruptedException {
		super(PHASE, new Phase1ModelTrainer(PHASE));
	}

	public static void main(String[] args) {
		try {
			new Phase4Calculator().calculate();
		} catch (IOException e) {
			getLogger().error(e);
	...
	public void calculate() throws IOException {
		...
		while ((strLine = br.readLine()) != null) {
			...
			if (s.equals("1") || s.equals("0")) {
				...
				try {
					String split = new SentenceSplitter().split(coreMap);
					coreMap = CoreNlpPipeline.getPipeline().process(split)
				...
				double resultSentiment = getResultSentiment(holder);
				if (s.equals("1")) {
					s = "Positive";
				} else {
					s = "Negative";
